...
- [x] **REQ-OBS-01**: Both services emit OpenTelemetry traces via `@opentelemetry/sdk-node` + `nestjs-otel`; spans propagate across HTTP, AMQP, and WebSocket boundaries via W3C TraceContext. _[P10-03 OTel NodeSDK literal-first `import "./tracing"` in both services' `main.ts`; ObservabilityModule composes `OpenTelemetryModule.forRoot({ metrics: { apiMetrics: { enable: true } } })`; auto-instrumentations cover http + nest + amqplib + pg; ADR-035 codifies SDK + Jaeger backend choice]_
...
- [x] **REQ-OBS-04**: All logs are structured JSON via `pino` + `nestjs-pino` with `correlationId` + `traceId` enrichment. _[P10-03 ObservabilityModule.LoggerModule.forRootAsync useFactory reads `trace.getActiveSpan()?.spanContext()` for traceId + spanId, plus ClsService for the saga correlationId; redacts `req.headers.authorization` + `req.headers.cookie`; pino-pretty transport gated behind `PINO_PRETTY=1` env]_
...
- [x] **REQ-CI-03**: README has CI status badges (build, tests, coverage). _[P10-09 README top line ships `[![CI](https://github.com/pedropaulobrasca/fullstack-challenge/actions/workflows/ci.yml/badge.svg)](https://github.com/pedropaulobrasca/fullstack-challenge/actions/workflows/ci.yml)`; Codecov badge deferred per 10-RESEARCH Deferred Ideas — separate Codecov enrollment out of scope for the submission]_

### Documentation (DOC)

- [x] **REQ-DOC-01**: README documents setup, decisions, trade-offs, architecture diagram, saga flow, provably-fair algorithm, scripts, env vars, troubleshooting. _[P10-09 README sections: Quick Start (one-command bootstrap with live URL table) + Architecture (mermaid graph TB of services + flow) + Saga Flow (mermaid sequenceDiagram showing one trace_id across HTTP+AMQP+WS for a single bet) + Provably Fair: Verify Outside the App (Phase 8 P08-09 preserved byte-stable) + Observability (Jaeger + Prometheus + Grafana per ADR-035) + ADR Catalogue (37-row generated table per ADR-036) + Scripts (root + per-service) + Env Vars (Phase 10 OTel/pino/Prometheus additions) + Troubleshooting (Pitfall 5/8 + D-03 polish defects)]_
- [x] **REQ-DOC-02**: Architecture Decision Records (ADRs) committed in `.planning/adrs/` (and surfaced in README) for each significant choice — ORM, money lib, outbox hand-roll vs library, raw `amqplib` + `@golevelup/nestjs-rabbitmq` split, server-tick rate, hash-chain depth, light-CQRS-no-ES, bet-202-cashout-200 asymmetry, single-lobby-vs-per-round-room. _[P10-09 37 ADRs in `.planning/adrs/` surfaced in README via `scripts/build-adr-index.ts` between sentinel markers, CI-gated by `bun run docs:adr-index:check` (ADR-036). Every significant decision listed in this requirement has a corresponding ADR — ORM (ADR-001), Money lib (ADR-002), outbox (ADR-007), amqplib split (ADR-008), server-tick rate (ADR-022), hash-chain depth (ADR-016), light CQRS (ADR-032), bet-202-cashout-200 (ADR-020), single lobby (ADR-021)]_
...
| REQ-OBS-01 | OpenTelemetry traces across HTTP/AMQP/WS | Done (Phase 10 P10-03) |
...
| REQ-OBS-04 | Structured JSON logs via `pino` + `nestjs-pino` with correlationId | Done (Phase 10 P10-03) |
...
| REQ-CI-03 | README status badges (build, tests, coverage) | Done (Phase 10 P10-09) |
| REQ-DOC-01 | README: setup, decisions, trade-offs, diagrams, troubleshooting | Done (Phase 10 P10-09) |
| REQ-DOC-02 | ADRs in `.planning/adrs/` surfaced in README | Done (Phase 10 P10-09) |
